...
desired_cap = {
   ...
   'os': 'Windows',
   'name': 'BStack-[Python] Smoke Test for travelnoire.com most popular section is as expected on desktop',
...
def page_load():
    try:
        WebDriverWait(driver, 20).until(ec.title_is("Travel Noire"))
    except TimeoutException:
        driver.execute_script(
          'browserstack_executor: {"action": "setSessionStatus", "arguments": '
          '{"status":"failed", "reason": for travelnoire.com, for web, took too long but no response, checking title"}}')
        driver.quit()


def verify_most_popular():
    print("in the function verify_most_popular section")
    most_popular = driver.find_element(By.XPATH, "//p[contains(text(),'Most Popular')]")
    actions = ActionChains(driver)
    actions.move_to_element(most_popular).perform()
    assert most_popular.is_displayed(), "'Most Popular' heading is not displayed"
    tmp = driver.find_elements(By.CSS_SELECTOR, "div.home__sidebar li")
    ...
    while count < temp_articles:
        ...
        time.sleep(1)
        temp_xpath = "//div[@class='color-white flex-full']//li[" + temp_string + "]/a[1]"
        ...
        temp_heading = article + " - Travel Noire"
...
def post_page_load_pop_up():
    ...
    try:
        ...
        print("travelnoire footer cookies pop-up does not exist")
...
def set_status():
    ...
    driver.execute_script(
      'browserstack_executor: {"action": "setSessionStatus", "arguments": '
      '{"status":"passed", "reason": ", for desktop, on travelnoire.com Most Popular section do work as expected"}}')
