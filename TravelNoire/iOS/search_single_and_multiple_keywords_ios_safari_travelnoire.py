...
desired_cap = {
  ...
  'os': 'iOS',
   'name': 'BStack-[Python] Smoke Test for travelnoire.com '
           'search functionality do work as expected for single and multiple keywords on ios safari',  # test name
...
def verify_nav_search_bar(search_text):
    ...
    if driver.title == "Travel Noire - Travel Noire":
        ...
        if page_header_error.is_displayed():
             driver.execute_script(
             'browserstack_executor: {"action": "setSessionStatus", "arguments": '
             '{"status":"failed", "reason": ", for ios safari, for the search text : ' + search_text +
