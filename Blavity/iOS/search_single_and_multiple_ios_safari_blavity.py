...
desired_cap = {
  ...
  'os': 'iOS',
   'name': 'BStack-[Python] Smoke Test for blavity.com for differnt search text working as expected or not on ios safari',
...
def verify_nav_search_bar(input):
    ...
    if input == 'culture':
        ...
        driver.execute_script(
          'browserstack_executor: {"action": "setSessionStatus", "arguments": '
          '{"status":"passed", "reason": ", for ios safari, for the search text culture on '
        ...
        driver.execute_script(
          'browserstack_executor: {"action": "setSessionStatus", "arguments": '
          '{"status":"failed", "reason": ", for ios safari, for search text Yolanda Baruch on'
