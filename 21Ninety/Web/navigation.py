...
desired_cap = {
   ...
   'os': 'Windows',
   'name': 'BStack-[Python] Smoke Test for 21ninety.com nav bar section is as expected or not on desktop',
...
def verify_nav_bar_presense():
    ...
    nav_bar_links = driver.find_element(By.XPATH, "//div[@class='container-fluid d-flex align-items-center']")
    print(nav_bar_links.text)
    assert nav_bar.is_displayed() and nav_bar_links.text is not None and nav_bar_links.text != "",\
        "navbar links are not present"
...
def set_status():
    ...
    driver.execute_script(
      'browserstack_executor: {"action": "setSessionStatus", "arguments": '
      '{"status":"passed", "reason": ", for desktop, on 21ninety.com navigation section do work as expected"}}')
